...
export default function AdminDashboard() {
    ...
    const saveBatchToFirestore = async (codes) => {
        ...
        try {
            ...
            // 2. global index docs - query by boxId field instead of using QR as document ID
            codes.forEach(qr => {
                batch.set(firestore().collection('scannedBoxIds').doc(), {
                    boxId: qr, // Store QR code as a field
                    batchId,
                    scannedAt: timestamp
                });
            });
    ...
    const handleVendorLinkSubmit = async () => {
        ...
        try {
            ...
            for (const packId of scannedFiberPacks) {
                // Query fiberPacks collection where boxId matches the scanned QR
                ...
                const packData = querySnapshot.docs[0].data();
                if (packData.weight) {
            ...
            const batch = firestore().batch();
            for (const packId of scannedFiberPacks) {
                // Find the document with matching boxId
                const querySnapshot = await firestore()
                    .collection('fiberPacks')
                    .where('boxId', '==', packId)
                    .get();
    ...
    // Updated barcode scanner handler to match your Firestore structure
    const handleBarcodeScanned = async ({ data }) => {
        if (isProcessingScan) return;
        
        if (!data || typeof data !== 'string') {
            Alert.alert("Invalid Scan", "Scanned QR code is empty or invalid.");
            return;
        }
        ...
        if (scannedCodes.includes(cleanedData)) {
            console.log("Duplicate scan in this session.");
            return;
        }
        ...
        try {
            // Query the scannedBoxIds collection to check if this boxId already exists
            const querySnapshot = await firestore()
                .collection('scannedBoxIds')
                .where('boxId', '==', cleanedData)
                .get();

            if (!querySnapshot.empty) {
                // Duplicate found - get the first matching document
                const existingDoc = querySnapshot.docs[0];
                const existingData = existingDoc.data();
                const scanDate = existingData.scannedAt 
                    ? existingData.scannedAt.toDate().toLocaleDateString() 
                    : 'N/A';
                
                Alert.alert(
                    'Duplicate Box',
                    `This box is already in batch ${existingData.batchId || 'Unknown'} (${scanDate})`
                );
                setIsProcessingScan(false);
                return;
            }

            // ✅ UNIQUE – add to session list
            const updated = [...scannedCodes, cleanedData];
            setScannedCodes(updated);

            // auto-save when last box is scanned
            if (updated.length === parseInt(boxCount, 10)) {
                setView('dashboard');
                await saveBatchToFirestore(updated);
            }
        } catch (e) {
